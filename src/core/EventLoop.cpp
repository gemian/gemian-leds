...
EventLoop::EventLoop(std::string const& name)
...
std::future<void> EventLoop::enqueue(std::function<void()> const& callback)
{
...
std::future<void> EventLoop::schedule_in(
...
void EventLoop::schedule_with_cancellation_in(
    std::chrono::milliseconds timeout,
    std::function<void()> const& callback,
